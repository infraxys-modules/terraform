if [[ "${D}initial_script_name" != terraform_* ]]; then
	log_info "Script '${D}initial_script_name' doesn't start with 'terraform_', so not initializing the Terraform environment";
else 	
